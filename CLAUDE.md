...
- Asset pipeline (later): same manifest-driven model as code, not a separate system --
  extractors run against the user's local ROM (gitignored output, like `build/`), and
  nothing gets committed unless it's genuinely worth extracting as source. Round-trips are
  enforced by the final sha1 compare.
