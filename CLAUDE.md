...
- **Audio: Krawall engine (confirmed, located in both ROMs).** Krawall is an XM/S3M module
  ...
  - Music/sample content lives as curated JSON+WAV under `data/audio/` (modules with inline
    ...
    committed. `tools/krawall_migrate.py` bootstraps it locally
    ...
    modding). `tools/pack_krawall.py` then packs this local, version-independent source
    ...
    vary per version). `tools/extract_krawall.py` (the old raw ROM -> `asm/krawall/<ver>/*.bin`
...
│                           #   tools/krawall_migrate.py, packed to per-version assembly
│                           #   by tools/pack_krawall.py
