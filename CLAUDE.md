...
- **Audio: Krawall engine (confirmed, located in both ROMs).** Krawall is an XM/S3M module
  ...
  - Music/sample data is located and bit-perfectly built via `unkrawerter` +
    `tools/extract_krawall.py` (pure deterministic struct parsing, not curated like code --
    see `docs/formats/krawall.md`). `regions.<ver>.txt` lists every region (address, name);
    the raw bytes themselves (`asm/krawall/<ver>/`) are gitignored and regenerated locally
    before each build (`just build` depends on `extract-krawall`) -- they're the game's
    actual copyrighted audio content, same footing as the baserom itself, hard rule 2. A
    separate, lossy `.xm` export exists for actually listening to/viewing the music
    (`just extract-music-xm`), never used by the build -- round-tripping `.xm` back through
    Krawall's own `krawerter` compiler was investigated but not adopted; see
    `docs/formats/krawall.md`'s Open questions.
...
- Asset pipeline: same manifest-driven model as code, not a separate system -- extractors
  ...
  way. What's NOT committed is the raw asset bytes themselves (regenerated locally before
  each build, like `build/`) -- actual game assets (art, audio, etc.) sit on the same footing
  as the baserom under hard rule 2, regardless of how mechanically they were located.
