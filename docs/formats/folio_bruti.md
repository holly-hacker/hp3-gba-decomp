...
`functions.us.cfg`; `InitMonsterBattleActor` is additionally matched in
`src/battle/`, the other two are not extracted, per
...
the pre-C JSON bootstrap also carried a `_description` field, dropped
here since it duplicates dialog text rather than ROM table data;
`_description` was `null` past index 53 anyway). Per-version `c-file`
rows (`0x0804F410`-`0x0804FA88` US, `0x0804F33C`-`0x0804F9B4` JP) place
the one source at each ROM's address, the same way `g_pMonsterDropTable`
and the shop-stock tables work.
