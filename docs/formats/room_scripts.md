...
| `0xA` (10) | `PlaySoundByIdCommand` | 1 (sound id) | Plays the id via `PlaySoundById` (US `0x0803FC68`) -- the same function `ShowRoomDialog` (opcode `5`) uses for its own open-dialog sound. Handler at US `0x0801C72C`. |
...
| `0x12` (18) | `SetAllQueuedMoveParams` | 6 (speed u16, posX u16, byte, byte) | Broadcasts the same parameters (position from `posX`, speed*30, state=3, plus two operand bytes) to every active slot (`0..DAT_03003378`) of the queued-object-move array `QueueTileObjectMove` (opcode `0x10`) populates. Handler at US `0x0801C04C`. |
...
| `0x17` (23) | `SetTileObjectShadow` | 7 (x, y, shadowX, shadowY, useShadowTile bool, field_0x65, field_0x68) | Resolves the tile object and sets its `pShadowObject` to either the default special-object slot `0` or a second resolved tile object (`shadowX`, `shadowY`) if `useShadowTile` is set, then sets `field_0x65`/`field_0x68` and attack-anim state `0x12`. Handler at US `0x0801C9B0`. |
...
| `0x24` (36) | `ShowBlackScreenTransition` | 4 (3 bytes forwarded to `PushGameMode_3`, unused) | Arms the yield state, ORs `g_dwGameModeFlags` bit `0x1` (the same "dialog open" bit `ShowRoomDialogBox_candidate` sets), then `PushGameMode_3(BlackScreen, ...)`. Handler at US `0x0801CBB4`. |
| `0x25` (37) | `PlayScreenTransitionEffect2` | 0 | Calls `FUN_0803d3e8` (US `0x0803D3E8`) with `(0x3f, 2)` -- identical constant arguments to `PlayScreenTransitionEffect` (opcode `0x26`)'s `FUN_0803D3B0(63, 2)` call, into a sibling transition-dispatch function; likely a variant (blocking/non-blocking, or fade direction) of the same screen-transition effect, not independently confirmed. Handler at US `0x0801CBE8`. |
...
| `0x2C` (44) | `SetTileObjectAnimStateValue` | 4 (x, y, animState, unused) | Resolves the tile `(x, y)` object and calls `SetFighterAttackAnimState_candidate(obj, animState)` with the state taken directly from the operand -- the generic, non-hardcoded sibling of `0xF`/`0x22` (`SetTileObjectAnimState`/`SetTileObjectAnimStateWithSpeed`), which pass fixed state constants. Handler at US `0x0801CCC8`. |
| `0x2D` (45) | `GrantRoomReward` | 2 (rewardId, variant byte) | Puts `g_pPlayerObject` into its "receive item" animation state (attack-anim state `0x17`, `field_0x92 = rewardId`) and calls the already-identified `GrantBattleReward(rewardId, count)` (count is a random 30-60 roll only for `rewardId 0x83`, else `1`) directly from a room script rather than a battle outcome. The `variant` byte selects whether `field_0x80` is additionally set to `0x42` afterward. Handler at US `0x0801CCEC`. |
...
| `0x35` (53) | `SetTileObjectAndLinkedActiveBit` | 3 (x, y, bool) | Resolves the tile object and sets/clears `Object+0xc` bit `0x1` per the `bool` operand, propagating the same set/clear to its linked object's (`field_0xa4`) `Object+0xc` bit `0x1` if present. Handler at US `0x0801CDCC`. |
...
| `0x3B` (59) | `SetTileObjectFacing` | 3 (x, y, facing byte) | Resolves the tile object, sets attack-anim state `3` (idle), and writes `Object->field_0x12` (facing, halved if `Object+0xc` bit `0x100000` is set). Handler at US `0x0801CEB0`. |
...
| `0x43` (67) | `SetTileObjectEffectSlot` | 3 (x, y, slot 2-bit value) | Resolves the tile object and writes `(slot & 3) << 2` into `Object->bFlagsAndEffectSlot_0xD5` bits `0xC`. Handler at US `0x0801CF64`. |
...
| `0x46` (70) | `RespawnRowAndRunChainCommand` | 2 (respawnRow, chainRow) | Thin wrapper over `RespawnRowAndRunChain_candidate(respawnRow, chainRow)` (US `0x08005B00`, shared with opcode `0xD`'s zero-speed branch). Handler at US `0x0801CFBC`. |
| `0x47` (71) | `StartMinigame` | 5 (minigameId, param, showHelp, pendingRow, pendingChain) | Latches `g_bPendingRoomScriptRow`/`g_bPendingRoomScriptChain`, then dispatches by `minigameId` (`0`=`WizardCrackerPopItMinigame`, `1`=`HippogriffGlideMinigame`, `2`=`RiddikulusMinigame`, `4`=`HarryVsDementorsMinigame`): if `showHelp` is set, pushes the shared `WizardCrackerPopItHelp` mode with `(6, minigameId, param)` instead of the minigame itself; otherwise pushes that minigame's own `GameMode` with `(6, 0, param)`, after setting a per-minigame "unlocked/seen" flag global (`DAT_03005228`/`DAT_03002040`/`DAT_03002E10` or `g_dwHippogriffForceOverworldExit`). Handler at US `0x0801C440`. |
| `0x48` (72) | `SetRoomRowObjectFlags` | 1 (row) | If `ShouldRunRoomScriptRow_candidate(row)` passes, loops every column in that row's tile-object table (`FUN_08005bcc(row)` columns) and ORs `0x82` into both the object's own `Object+0xc` flags and its linked object's (`field_0xa4`) `Object+0xc` flags -- the row-batch version of `SetTileObjectFlagsPair` (opcode `1`), which does the same to a single tile. Handler at US `0x0801CFCC`. |
...
| `0x4E` (78) | `SetHeaderFlag` | 1 (index 0-4) | ORs one of 5 bits (`0x2`, `0x4`, `0x8`, `0x10`, `0x20`, selected by `index`) into `g_bHeaderFlags`; any other index is a no-op. Handler at US `0x0801D0C8`. |
| `0x4F` (79) | `ShowRewardPickupMessage` | 4 (id, padding, padding, unused) | Arms the yield flag, plays a fixed sound (`0x1A`), resolves `id` to dialog text (`FUN_08026b8c` + `GetDialogText`, once into a text-prep call `FUN_08021100`/`FUN_08021148`), then opens dialog block `0x28D` (string 972, **"@3 has been added to your Inventory."**) if `id < 0x4F`, else block `0x295` (string 979, **"@3 has been added to your Folio Universitas."**) -- decoded from the real string table, not inferred from the block shape. A generic reward-pickup announcement (item vs. Folio card), matching `GrantBattleReward`'s own item-vs-card threshold. Handler at US `0x0801D138`. |
...
| `0x52` (82) | `SetPauseMenuLocked` | 1 (value) | `DAT_03003b60 = value` -- gates the overworld Start/Select menu-open check (`FUN_0802aef8`): non-zero blocks opening the Options/InGameMenu mode and plays a "denied" sound instead. Handler at US `0x0801D25C`. |
...
| `0x55` (85) | `ShowMinigameUnlockedMessage` | 4 (id, padding, padding, unused) | If `id == 3`, opens block `0x299` directly -- string 934, **"Tea Leaf Divination can now be accessed from the Mini-Games menu found on the Title Screen."** (the minigame-unlock announcement [`save.md`](save.md)'s `SaveGameHeaderBits` bits `0x02`/`0x04`/`0x08`/`0x10` already document); otherwise looks up dialog text at a fixed `+0xa4a` string-table offset from `id` and opens block `0x292` -- string 977, **"You've unlocked: @3."** A generic minigame-unlocked announcement, `id` selecting which of the 4 minigames. Handler at US `0x0801BEF8`. |
| `0x56` (86) | `ShowPartyLevelUpMessage` | 4 (id, padding, padding, unused) | Calls `FUN_08021184(id)`/`FUN_080211ac(id)` directly (no `GetDialogText` lookup -- `id` is not a name-table index here), then opens fixed dialog block `0x293` -- string 978, **"All members of your party level up by @3!"** -- `id` is the level-up count. Pairs with `GrantPartyLevelUps` (opcode `0x57`). Handler at US `0x0801D2C8`. |
...
`FUN_0800A3AC`. Neither register chain has been traced into a stable,
