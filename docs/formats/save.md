...
| `0x03002614` (`g_dwOverworldMonstersDisabled`) | `flOverworldMonstersDisabled` | Per the user: disables overworld random encounters with regular monsters (bosses still trigger). Packed as a single bit, not a byte. **PROVEN** against code: `InitializeRoomMode` only calls `SpawnOverworldMonsterEncounters` on room load along the branch taken when this flag is `0`; the nonzero branch restores room object state but never spawns encounters. Set/cleared by room-script opcodes `SetOverworldMonstersDisabled`/`ClearOverworldMonstersDisabled` (`0x4A`/`0x4B`, see [`room_scripts.md`](room_scripts.md)) -- the only located writers besides save load/new-game reset. |
